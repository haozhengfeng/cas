 ...
 *
 * 验证凭证并且取回这些凭证的 授予凭证（TGT）。
 * 如果是一个更新请求，同时生成需要的服务票证(ST)。
 *
...
//        }
        ...
        try {
            ...
            WebUtils.getHttpServletRequest(context).getSession().setAttribute("session_user_name", ((UsernamePasswordCredential)credential).getUsername());
